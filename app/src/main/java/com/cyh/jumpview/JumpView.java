...
public class JumpView extends View {
    ...
    private void initJumpAnimators() {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mPointOffset = mWidth / mPointCount - mRadius * 2;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int index = 0; index < points.length; index += 2) {
            ...
            float offset = -mPointOffset / 2;
            currentR.top = points[index + 1] - mRadius + shift;
            currentR.bottom = points[index + 1] + mRadius + shift;
            currentR.left = points[index] - mRadius + offset;
            currentR.right = points[index] + mRadius + offset;
            canvas.drawArc(currentR, 0, 360, true, mPaint);
        }
    }
    
    public boolean isRunning() {
        return mCanRunning.get();
    }
    
    public void resume() {
        Log.d(LOG_TAG, "resume: " + mCanRunning);
        if (mCanRunning.compareAndSet(false, true)) {
            for (ValueAnimator animator : mJumpAnimators) {
    ...
    public void pause() {
        Log.d(LOG_TAG, "pause: " + mCanRunning);
